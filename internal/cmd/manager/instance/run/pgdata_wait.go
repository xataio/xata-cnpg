...
const (
	pgdataWaitTimeout  = 30 * time.Second
	pgdataWaitInterval = 10 * time.Millisecond
...
// slightly after the pod starts; this function polls until PGDATA or the
// marker file appears, up to a 30-second timeout.
func waitForPGData(ctx context.Context, pgData string) error {
	...
	for {
		...
		case <-deadline:
			return fmt.Errorf("timed out after %s waiting for PGDATA directory %q to appear", pgdataWaitTimeout, pgData)
