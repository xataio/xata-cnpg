...
var _ = Describe("QueryRunner tests", func() {
	...
	Context("receiving query data from the database", func() {
		...
		It("should compute the default metrics successfully", func() {
			...
			dbMock.ExpectExec("SET search_path = pg_catalog").WillReturnResult(sqlmock.NewResult(0, 1))
			dbMock.ExpectExec("SET LOCAL session_authorization TO pg_monitor").WillReturnResult(sqlmock.NewResult(0, 1))
