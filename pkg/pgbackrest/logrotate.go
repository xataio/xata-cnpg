...
// one) and truncated in place. Copy-then-truncate is required rather than
// rename: the long-running TLS server keeps all-server.log open, and a rename
// would leave it appending to the rotated file. Lines written between the
// copy and the truncate are lost; that window is negligible for log data.
func RotateLogs(pgDataPath string) error {
